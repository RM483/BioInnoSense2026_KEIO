...
### 読み出し回路（実機の実体配線。2026-08 実装・実測済み）
```
VC(5V) ─ RL(10k) ─┬─ VS節点 ─ Rs(SB19: S極〜ヒーター中点 ≈0.45V)
                   └─→ MCP6022(電圧フォロワ,×1) ─ RC(1kΩ/1µF LPF) ─→ R4 A0 (14bit ADC)
ヒータ VH=0.9V ← LT3080 レギュレータ(常時)  ＊実測0.49W発熱・ヒートシンク不要
任意: VH監視 ─ RC(1kΩ/1µF LPF) ─→ R4 A2
```
- **注意: RLが5V側**（3本足センサはヒーターがGND共通のため、RLはS極側にしか置けない）。
  `VS = VMID + (VC−VMID)·Rs/(Rs+RL)`（VMID≈ヒーター中点≈0.45V）。
  **Rsが下がる(H2↑)と VS は「下がる」**（旧図と逆。ソフトは `config.h: RL_HIGH_SIDE=1` で対応済み）。
- **ADC基準=VC(AVCC=5V) にすると VS/VC が比率になり電源変動がキャンセル**（ratiometric）。
  → `Rs = RL · (ratio − VMID/VC)/(1 − ratio)`（`sensor_sb19.h`）。
- 実測基準点: クリーンエア（慣らし前）で VS/VC = 3.11/4.68 → Rs ≈ 17 kΩ。

### R4 への配線
| 信号 | R4 ピン | 備考 |
|---|---|---|
| VS（フォロワ+RC後） | **A0** | センサ出力（基板J2ピン） |
| VH監視(任意) | A2 | ヒーター0.9Vのモニタ（基板J3ピン）。A1はVC監視用に空けてある |
| VC監視(任意) | A1 | ratiometric運用なら未使用可（現状未実装） |
